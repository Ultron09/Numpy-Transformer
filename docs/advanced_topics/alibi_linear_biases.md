# 🧠 Technical Specification: ALiBi Linear Biases

## 📌 Executive Summary
Attention with Linear Biases for zero-shot context length extrapolation.

---

## 🔬 Mathematical Formulation & Algorithmic Design

$$\mathcal{L}_{\text{alibi_linear_biases}} = \sum_{i=1}^{N} \operatorname{Softmax}\left( \frac{Q_i K_i^T}{\sqrt{d_k}} \right) V_i$$
...
def compute_alibi_linear_biases(inputs: np.ndarray, scaling_factor: float = 1.0) -> np.ndarray:
    """
    Reference implementation for ALiBi Linear Biases.
    """
