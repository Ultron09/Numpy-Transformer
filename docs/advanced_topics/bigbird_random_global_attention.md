# 🧠 Technical Specification: BigBird Random Global Attention

## 📌 Executive Summary
Theoretical universal approximator sparse graph attention.

---

## 🔬 Mathematical Formulation & Algorithmic Design

$$\mathcal{L}_{\text{bigbird_random_global_attention}} = \sum_{i=1}^{N} \operatorname{Softmax}\left( \frac{Q_i K_i^T}{\sqrt{d_k}} \right) V_i$$
...
def compute_bigbird_random_global_attention(inputs: np.ndarray, scaling_factor: float = 1.0) -> np.ndarray:
    """
    Reference implementation for BigBird Random Global Attention.
    """
