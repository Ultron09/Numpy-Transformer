# 🧠 Technical Specification: Sandwich Normalization

## 📌 Executive Summary
Dual pre- and post-layer normalization for training stabilization.

---

## 🔬 Mathematical Formulation & Algorithmic Design

$$\mathcal{L}_{\text{sandwich_normalization}} = \sum_{i=1}^{N} \operatorname{Softmax}\left( \frac{Q_i K_i^T}{\sqrt{d_k}} \right) V_i$$
...
def compute_sandwich_normalization(inputs: np.ndarray, scaling_factor: float = 1.0) -> np.ndarray:
    """
    Reference implementation for Sandwich Normalization.
    """
