# 🧠 Technical Specification: Weight Only 4-bit Quantization

## 📌 Executive Summary
Block-wise scales and zero-point calibration for FP32 down-casting.

---

## 🔬 Mathematical Formulation & Algorithmic Design

$$\mathcal{L}_{\text{weight_only_4_bit_quantization}} = \sum_{i=1}^{N} \operatorname{Softmax}\left( \frac{Q_i K_i^T}{\sqrt{d_k}} \right) V_i$$
...
def compute_weight_only_4_bit_quantization(inputs: np.ndarray, scaling_factor: float = 1.0) -> np.ndarray:
    """
    Reference implementation for Weight Only 4-bit Quantization.
    """
