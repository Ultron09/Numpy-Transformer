# 🧠 Technical Specification: IA3 Vector Scaling Adaptation
...
$$\mathcal{L}_{\text{ia3_vector_scaling_adaptation}} = \sum_{i=1}^{N} \operatorname{Softmax}\left( \frac{Q_i K_i^T}{\sqrt{d_k}} \right) V_i$$
...
def compute_ia3_vector_scaling_adaptation(inputs: np.ndarray, scaling_factor: float = 1.0) -> np.ndarray:
    """
    Reference implementation for IA3 Vector Scaling Adaptation.
    """
