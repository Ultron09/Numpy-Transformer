# 🧠 Technical Specification: QLoRA Double Quantization

## 📌 Executive Summary
NF4 NormalFloat data type and double quantization for 4-bit fine-tuning.

---

## 🔬 Mathematical Formulation & Algorithmic Design

$$\mathcal{L}_{\text{qlora_double_quantization}} = \sum_{i=1}^{N} \operatorname{Softmax}\left( \frac{Q_i K_i^T}{\sqrt{d_k}} \right) V_i$$
...
def compute_qlora_double_quantization(inputs: np.ndarray, scaling_factor: float = 1.0) -> np.ndarray:
    """
    Reference implementation for QLoRA Double Quantization.
    """
