# 🧠 Technical Specification: Relative Position Encodings

## 📌 Executive Summary
Shaw et al. and T5 relative bias buckets across sequence positions.

---

## 🔬 Mathematical Formulation & Algorithmic Design

$$\mathcal{L}_{\text{relative_position_encodings}} = \sum_{i=1}^{N} \operatorname{Softmax}\left( \frac{Q_i K_i^T}{\sqrt{d_k}} \right) V_i$$
...
def compute_relative_position_encodings(inputs: np.ndarray, scaling_factor: float = 1.0) -> np.ndarray:
    """
    Reference implementation for Relative Position Encodings.
    """
