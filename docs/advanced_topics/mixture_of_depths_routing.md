# 🧠 Technical Specification: Mixture of Depths Routing

## 📌 Executive Summary
Top-k token capacity limiting for dynamic compute allocation in transformer blocks.

---

## 🔬 Mathematical Formulation & Algorithmic Design

$$\mathcal{L}_{\text{mixture_of_depths_routing}} = \sum_{i=1}^{N} \operatorname{Softmax}\left( \frac{Q_i K_i^T}{\sqrt{d_k}} \right) V_i$$
...
def compute_mixture_of_depths_routing(inputs: np.ndarray, scaling_factor: float = 1.0) -> np.ndarray:
    """
    Reference implementation for Mixture of Depths Routing.
    """
